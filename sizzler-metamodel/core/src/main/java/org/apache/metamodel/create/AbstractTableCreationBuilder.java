...
/**
 * Abstract {@link TableCreationBuilder} implementation, provided as convenience for
 * {@link TableCreatable} implementations. Handles all the building operations, but not the commit
 * operation.
 */
public abstract class AbstractTableCreationBuilder<U extends UpdateCallback> implements
  ...
  public AbstractTableCreationBuilder(U updateCallback, Schema schema, String name) {
    if (schema != null && schema.getTableByName(name) != null) {
      throw new IllegalArgumentException("A table with the name '" + name
