...
/**
 * Abstract {@link DataSet} implementation for use in scenarios where a pull-oriented style of
 * reading data is not supported. This implementation instead allows a publshing action to publish
 * rows to the dataset in a blocking manner, and thereby to adapt without having to load all rows
 * into memory.
 */
public final class RowPublisherDataSet extends AbstractDataSet {
  ...
  public RowPublisherDataSet(SelectItem[] selectItems, int maxRows,
      Action<RowPublisher> publishAction, Closeable... closeables) {
    super(selectItems);
    _maxRows = maxRows;
