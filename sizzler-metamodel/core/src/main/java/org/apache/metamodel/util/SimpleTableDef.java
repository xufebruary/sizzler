...
/**
 * Represents a table definition to be used in scenarios where a {@link DataContext} is unable to
 * detect/discover the table structure and needs some basic input around expected table structures.
 */
public class SimpleTableDef implements Serializable, HasName {
    ...
    if (columnTypes == null || columnTypes.length == 0) {
      ...
      if (columnNames.length != columnTypes.length) {
        throw new IllegalArgumentException(
            "Property names and column types cannot have different lengths (found "
                + columnNames.length + " and " + columnTypes.length + ")");
      }
    }
  ...
  public String toString() {
    return "SimpleTableDef[name=" + _name + ",columnNames=" + Arrays.toString(_columnNames)
        + ",columnTypes=" + Arrays.toString(_columnTypes) + "]";
  }

  /**
   * Creates a {@link MutableTable} based on this {@link SimpleTableDef}. Note that the created
   * table will not have any schema set.
   *
