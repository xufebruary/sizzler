...
/**
 * Compares dates of various formats. Since this class has unchecked generic conversion it can
 * compare java.util.Date, java.sql.Date, java.sql.Time, java.util.Calendar, Date-formatted strings
 * etc.
 */
public final class TimeComparator implements Comparator<Object> {
   ...
   * by shaoqiang.guo
   */
  private static final String[] prototypePatterns = {"yyyy-MM-dd HH:mm:ss.SSS",
      "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm", "HH:mm:ss.SSS", "yyyy-MM-dd", "dd-MM-yyyy",
      "yy-MM-dd", "MM-dd-yyyy", "HH:mm:ss", "HH:mm", "yyyyMMdd HH:mm:ss.SSS", "yyyyMMdd HH:mm:ss",
      "yyyyMMdd HH:mm", "yyyyMMdd", "ddMMyyyy", "yyMMdd", "MMddyyyy", "yyyy/MM/dd HH:mm:ss.SSS",
      "yyyy/MM/dd HH:mm:ss", "yyyy/MM/dd HH:mm", "yyyy/MMdd", "dd/MM/yyyy", "yyMMdd", "MMddyyyy",
      "yyyy-MM", "yyyy/MM"};
  ...
  private static Date convertFromString(final String value) {
    ...
    try {
      ...
      SimpleDateFormat dateFormat =
          new SimpleDateFormat("EEE MMM dd HH:mm:ss zzz yyyy", dateFormatSymbols);
  ...
  private static Date convertFromNumber(Number value) {
     ...
     * by shaoqiang.guo
     */
    // test if the number is actually a format of the type yyyyMM
    if (stringValue.length() == 6) {
      try {
        return DateUtils.createDateFormat("yyyyMM").parse(stringValue);
      } catch (Exception e) {
        // do nothing, proceed to next method of conversion
      }
    }
     ...
     * by shaoqiang.guo
     */
    // test if the number is actually a format of the type yyyy
    if (stringValue.length() == 4) {
      try {
        return DateUtils.createDateFormat("yyyy").parse(stringValue);
      } catch (Exception e) {
        // do nothing, proceed to next method of conversion
      }
    }
