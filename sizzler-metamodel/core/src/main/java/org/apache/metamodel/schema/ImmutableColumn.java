 ...
 * <p/>
 * It is not intended to be instantiated on it's own. Rather, use the constructor in
 * ImmutableSchema.
 *
 * @see ImmutableSchema
 */
public final class ImmutableColumn extends AbstractColumn implements Serializable {
   ...
   */
  public ImmutableColumn(String name, ColumnType type, Table table, int columnNumber,
      Integer columnSize, String nativeType, Boolean nullable, String remarks, boolean indexed,
      String quote, boolean primaryKey) {
    this.name = name;
    this.type = type;
    this.table = table;
  ...
  protected ImmutableColumn(Column column, ImmutableTable table) {
    this(column.getName(), column.getType(), table, column.getColumnNumber(), column
        .getColumnSize(), column.getNativeType(), column.isNullable(), column.getRemarks(), column
        .isIndexed(), column.getQuote(), column.isPrimaryKey());
  }
