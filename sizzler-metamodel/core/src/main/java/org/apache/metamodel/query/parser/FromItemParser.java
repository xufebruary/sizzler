...
final class FromItemParser implements QueryPartProcessor {
  ...
  private FromItem parseAllJoinItems(final String itemToken) {
    ...
    for (int i = 0; i < joinSplit.length - 1; i++) {
      ...
      String rightPart =
          (i + 1 == joinSplit.length - 1) ? joinSplit[i + 1] : joinSplit[i + 1].substring(0,
  ...
  private FromItem parseJoinItem(final FromItem leftFromItem, final String itemToken,
      Set<FromItem> fromItems) {
