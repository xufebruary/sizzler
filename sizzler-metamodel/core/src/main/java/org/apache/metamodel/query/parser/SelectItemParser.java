...
public final class SelectItemParser implements QueryPartProcessor {
   ...
   * {@link #parse(String, String)} method, this method will not actually add the selectitem to the
   * query.
   *
   * @param expression
   * @return
   * @throws MultipleSelectItemsParsedException if an expression yielding multiple select-items
   *         (such as "*") was passed in the expression
   */
  public SelectItem findSelectItem(String expression) throws MultipleSelectItemsParsedException {
    ...
    if (startParenthesis > 0 && expression.endsWith(")")) {
      ...
      final String functionName =
          expression.substring(
              (functionApproximation ? SelectItem.FUNCTION_APPROXIMATION_PREFIX.length() : 0),
              startParenthesis);
    ...
    if (fromItem != null) {
      if ("*".equals(columnName)) {
        ...
        final SelectItem subQuerySelectItem =
            new SelectItemParser(subQuery, _allowExpressionBasedSelectItems)
    ...
    // select item
    final SelectItem aliasSelectItem =
        MetaModelHelper.getSelectItemByAlias(_query, unmodifiedExpression);
