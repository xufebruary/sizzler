...
public final class DefaultRow extends AbstractRow implements Row {
  ...
  public DefaultRow(DataSetHeader header, Object[] values, Style[] styles) {
    ...
    if (header.size() != values.length) {
      throw new IllegalArgumentException("Header size and values length must be equal. "
          + header.size() + " select items present in header and encountered these values: "
          + Arrays.toString(values));
    }
    if (styles != null) {
      if (values.length != styles.length) {
        throw new IllegalArgumentException("Values length and styles length must be equal. "
            + values.length + " values present and encountered these styles: "
   ...
   *
   * @param items the array of SelectItems
   * @param values the array of values
   * @deprecated use {@link #DefaultRow(DataSetHeader, Object[])} or
   *             {@link #DefaultRow(DataSetHeader, Object[], Style[])} instead.
   */
  @Deprecated
   ...
   * @param items the array of SelectItems
   * @param values the array of values
   * @param styles an optional array of styles
   * @deprecated use {@link #DefaultRow(DataSetHeader, Object[])} or
   *             {@link #DefaultRow(DataSetHeader, Object[], Style[])} instead.
   */
  @Deprecated
   ...
   *
   * @param items the list of SelectItems
   * @param values the array of values
   * @deprecated use {@link #DefaultRow(DataSetHeader, Object[])} or
   *             {@link #DefaultRow(DataSetHeader, Object[], Style[])} instead.
   */
  ...
  /**
   * Method invoked by the Java serialization framework while deserializing Row instances. Since
   * previous versions of MetaModel did not use a DataSetHeader, but had a reference to a
   * List<SelectItem>, this deserialization is particularly tricky. We check if the items variable
   * is there, and if it is, we convert it to a header instead.
   *
   * @param stream
   * @throws Exception
   */
