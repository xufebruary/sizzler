...
/**
 * Abstract exception type that represents exceptions that occur when reading a data format which
 * contain formatting errors or inconsistencies in on or more rows.
 * <p/>
 * Typically {@link InconsistentRowFormatException}s are thrown when calling {@link DataSet#next()}.
 * <p/>
 * All {@link InconsistentRowFormatException}s are optional, meaning that you can turn them on and
 * off. When turned off the result of {@link #getProposedRow()} will be used transparently instead
...
public abstract class InconsistentRowFormatException extends MetaModelException {
   ...
   *
   * @return a row object which represents the {@link Row} as MetaModel would gracefully interpret
   *         it.
   */
