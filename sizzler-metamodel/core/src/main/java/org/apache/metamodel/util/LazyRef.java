...
public abstract class LazyRef<E> implements Ref<E> {
  ...
  /**
   * Requests an asynchronous load of the lazy reference. If not already loaded, this will cause
   * another thread to load the reference, typically to make it immediately available for later
   * evaluation.
   *
   * @param errorAction an optional error action to invoke if an exception is thrown during loading
   *        of the reference.
   */
