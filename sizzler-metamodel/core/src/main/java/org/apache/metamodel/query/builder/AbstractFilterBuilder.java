...
/**
 * Abstract implementation of {@link FilterBuilder} interface. All built filters are channeled to
 * the {@link #applyFilter(FilterItem)} method which needs to be implemented by concrete
 * implementations.
 */
public abstract class AbstractFilterBuilder<B> implements FilterBuilder<B> {
  ...
  public B differentFrom(Column column) {
    ...
    return applyFilter(new FilterItem(_selectItem, OperatorType.DIFFERENT_FROM, new SelectItem(
        column)));
  }
  ...
  public B greaterThan(Column column) {
    ...
    return applyFilter(new FilterItem(_selectItem, OperatorType.GREATER_THAN,
        new SelectItem(column)));
  }
  ...
  public B lessThan(Object obj) {
    ...
    throw new UnsupportedOperationException("Argument must be a Number, Date or String. Found: "
        + obj);
  }
  ...
  public B greaterThanOrEquals(Column column) {
    ...
    return applyFilter(new FilterItem(_selectItem, OperatorType.GREATER_THAN_OR_EQUAL,
        new SelectItem(column)));
  }
  ...
  public B lessThanOrEquals(Column column) {
    ...
    return applyFilter(new FilterItem(_selectItem, OperatorType.LESS_THAN_OR_EQUAL, new SelectItem(
        column)));
  }
