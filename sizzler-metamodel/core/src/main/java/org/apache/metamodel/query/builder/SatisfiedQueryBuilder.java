...
public interface SatisfiedQueryBuilder<B extends SatisfiedQueryBuilder<?>> {
  ...
  /**
   * Gets the built query as a {@link Query} object. Typically the returned query will be a clone of
   * the built query to prevent conflicting mutations.
   *
  ...
  /**
   * Executes the built query. This call is similar to calling {@link #toQuery()} and then
   * {@link DataContext#executeQuery(Query)}.
   *
