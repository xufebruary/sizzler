...
/**
 * Abstract implementation of the {@link UpdateCallback} interface. Implements only the data store
 * agnostic methods.
 */
public abstract class AbstractUpdateCallback implements UpdateCallback {
  ...
  public TableDropBuilder dropTable(Schema schema, String tableName)
    ...
    if (table == null) {
      throw new IllegalArgumentException("Nu such table '" + tableName + "' found in schema: "
          + schema + ". Available tables are: " + Arrays.toString(schema.getTableNames()));
    }
    return dropTable(table);
  }
