...
/**
 * Represents a clause of filters in the query. This type of clause is used for the WHERE and HAVING
 * parts of an SQL query.
 * <p/>
 * Each provided FilterItem will be evaluated with the logical AND operator, which requires that all
 * filters are applied. Alternatively, if you wan't to use an OR operator, then use the appropriate
 * constructor of FilterItem to create a composite filter.
 *
 * @see FilterItem
 */
public class FilterClause extends AbstractQueryClause<FilterItem> {
  ...
  /**
   * Traverses the items and evaluates whether or not the given column is referenced in either of
   * them.
   *
   * @param column
