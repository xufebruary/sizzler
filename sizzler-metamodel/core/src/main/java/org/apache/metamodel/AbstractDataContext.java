...
/**
 * Abstract implementation of the DataContext interface. Provides convenient implementations of all
 * trivial and datastore-independent methods.
 */
public abstract class AbstractDataContext implements DataContext {
  ...
  private final ConcurrentMap<String, Schema> _schemaCache =
      new ConcurrentHashMap<String, Schema>();
  private final Comparator<? super String> _schemaNameComparator = SchemaNameComparator
      .getInstance();
  ...
  /**
   * Method invoked when schemas have been refreshed using {@link #refreshSchemas()}. Can be
   * overridden to add callback functionality in subclasses.
   */
      ...
      if (schemas.length == 1) {
        ...
        for (int i = 0; i < schemas.length; i++) {
          ...
          if (schema != null) {
            ...
            final boolean isInformationSchema =
                name.startsWith("information") && name.endsWith("schema");
    ...
    if (schema == null) {
      ...
      if (schema != null) {
        Schema existingSchema =
            _schemaCache.putIfAbsent(getSchemaCacheKey(schema.getName()), schema);
        if (existingSchema != null) {
   ...
   * @param columnNameSearch the column name as it should be searched for (either the same as
   *        original, or lower case in case of case-insensitive search)
   * @return
   */
  private Column searchColumn(String schemaNameSearch, String columnNameOriginal,
  ...
  /**
   * Gets schema names from the non-abstract implementation. These schema names will be cached
   * except if the {@link #refreshSchemas()} method is called.
   *
  ...
  /**
   * Gets a specific schema from the non-abstract implementation. This schema object will be cached
   * except if the {@link #refreshSchemas()} method is called.
   *
