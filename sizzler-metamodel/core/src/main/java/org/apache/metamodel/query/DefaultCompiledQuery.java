...
/**
 * Represents a default implementation of the {@link CompiledQuery} interface. This implementation
 * does not actually do anything to prepare the query, but allows creating a clone of the
 * originating query with the parameters replaced by values.
 */
public class DefaultCompiledQuery implements CompiledQuery {
  ...
  private void replaceParametersInQuery(Object[] values, AtomicInteger parameterIndex,
  ...
  private void replaceParametersInFromClause(Object[] values, AtomicInteger parameterIndex,
      Query originalQuery, Query newQuery) {
    ...
    for (FromItem fromItem : fromItems) {
      ...
      if (subQuery != null) {
        ...
        newQuery.getFromClause()
            .addItem(i, new FromItem(newSubQuery).setAlias(fromItem.getAlias()));
      }
      i++;
    }
  }
    ...
    if (item.isCompoundFilter()) {
      ...
      if (item.getOperand() instanceof QueryParameter) {
        ...
        final FilterItem newFilter =
            new FilterItem(item.getSelectItem(), item.getOperator(), newOperand);
        return newFilter;
      } else {
        return item;
      }
