...
public class FilterItem extends BaseObject implements QueryItem, Cloneable, IRowFilter {
  ...
  private List<FilterItem> _childItems;
   ...
   */
  private FilterItem(SelectItem selectItem, OperatorType operator, Object operand,
   ...
   * @param operator The operator to use. Can be OperatorType.EQUALS_TO,
   *        OperatorType.DIFFERENT_FROM, OperatorType.GREATER_THAN,OperatorType.LESS_THAN
   *        OperatorType.GREATER_THAN_OR_EQUAL, OperatorType.LESS_THAN_OR_EQUAL
   * @param operand The operand. Can be a constant like null or a String, a Number, a Boolean, a
   *        Date, a Time, a DateTime. Or another SelectItem
   * @throws IllegalArgumentException if the SelectItem is null or if the combination of operator
   *         and operand does not make sense.
   */
  public FilterItem(SelectItem selectItem, OperatorType operator, Object operand)
    ...
    if (_operator == OperatorType.LIKE || _operator == OperatorType.NOT_LIKE
        || _operator == OperatorType.START_LIKE || _operator == OperatorType.END_LIKE
        || _operator == OperatorType.NOT_START_LIKE || _operator == OperatorType.NOT_END_LIKE) {
  ...
  /**
   * Creates a single unvalidated filter item based on a expression. Expression based filters are
   * typically NOT datastore-neutral but are available for special "hacking" needs.
   * <p/>
   * Expression based filters can only be used for JDBC based datastores since they are translated
   * directly into SQL.
   *
  ...
  /**
   * Creates a composite filter item based on other filter items. Each provided filter items will be
   * OR'ed meaning that if one of the evaluates as true, then the composite filter will be evaluated
   * as true
   *
  ...
  /**
   * Creates a compound filter item based on other filter items. Each provided filter item will be
   * combined according to the {@link LogicalOperator}.
   *
  ...
  /**
   * Creates a compound filter item based on other filter items. Each provided filter items will be
   * OR'ed meaning that if one of the evaluates as true, then the compound filter will be evaluated
   * as true
   *
   * @param items an array of items to include in the composite
   */
    ...
    if (_childItems == null) {
      ...
      if (_operand == null && _operator == OperatorType.EQUALS_TO) {
        ...
        if (operand instanceof SelectItem) {
          final String selectItemString =
              ((SelectItem) operand).getSameQueryAlias(includeSchemaInColumnPaths);
  ...
  public boolean evaluate(Row row) {
    ...
    if (_childItems == null) {
      ...
      /**
       * start 新增根据字段类型判断是否需要将过滤条件转成Date类型。
       * @author shaoqiang.guo
       */
      ColumnType columnType = _selectItem.getColumn().getType();
      if (ColumnType.DATE.equals(columnType) || ColumnType.DATETIME.equals(columnType)
          || ColumnType.TIME.equals(columnType)) {
        operandValue = TimeComparator.toDate(operandValue);
        selectItemValue = TimeComparator.toDate(selectItemValue);
      }
      /**
       ********** end
       */
  ...
  private boolean compare(Object selectItemValue, Object operandValue) {
    ...
    } else if (_operator == OperatorType.NOT_LIKE || _operator == OperatorType.START_LIKE
        || _operator == OperatorType.END_LIKE || _operator == OperatorType.NOT_START_LIKE
        || _operator == OperatorType.NOT_END_LIKE) {
      WildcardPattern matcher = new WildcardPattern((String) operandValue, '%');
      return matcher.customizeMatches((String) selectItemValue, _operator);
  ...
  /**
   * Gets the {@link FilterItem}s that this filter item consists of, if it is a compound filter
   * item.
   *
  ...
  /**
   * Determines whether this {@link FilterItem} is a compound filter or not (ie. if it has child
   * items or not)
   */
