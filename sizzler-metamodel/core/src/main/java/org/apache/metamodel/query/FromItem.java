 ...
 * <lI>join FROMs with an ON clause (eg. "FROM products p INNER JOIN orders o ON p.id =
 * o.product_id")</li>
...
public class FromItem extends BaseObject implements QueryItem, Cloneable {
  ...
  /**
   * Constructor for join FROM clauses that join two tables using their relationship. The primary
   * table of the relationship will be the left side of the join and the foreign table of the
   * relationship will be the right side of the join.
   *
   ...
   */
  public FromItem(JoinType join, FromItem leftSide, FromItem rightSide, SelectItem[] leftOn,
      SelectItem[] rightOn) {
    _join = join;
    _leftSide = leftSide;
  ...
  /**
   * Creates a single unvalidated from item based on a expression. Expression based from items are
   * typically NOT datastore-neutral but are available for special "hacking" needs.
   * <p/>
   * Expression based from items can only be used for JDBC based datastores since they are
   * translated directly into SQL.
   *
  ...
  /**
   * Gets the alias of a table, if it is registered (and visible, ie. not part of a sub-query) in
   * the FromItem
   *
