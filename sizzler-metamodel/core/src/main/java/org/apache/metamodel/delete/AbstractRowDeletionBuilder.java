...
public abstract class AbstractRowDeletionBuilder implements RowDeletionBuilder {
  ...
  /**
   * Determines if a row should be deleted or not (can be used by subclasses as a convenient
   * determinator).
   *
   * @param row
  ...
  /**
   * Convenience method to tell subclasses if the delete operation represents a full table truncate
   * operation. Usually such operations can be optimized by simply removing the table (and maybe
   * restoring similar headers in a new table).
   *
   * @return
   */
  ...
  @Override
  public String toSql() {
    return "DELETE FROM " + _table.getQualifiedLabel()
        + new FilterClause(null, " WHERE ").addItems(_whereItems);
  }
}
