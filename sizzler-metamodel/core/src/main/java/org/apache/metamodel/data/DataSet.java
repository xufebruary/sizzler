...
/**
 * Represents a tabular DataSet where values are bound to columns and rows. A DataSet works
 * similarly to a slightly modularized ResultSet when you traverse it - use the next() method to
 * loop through the rows of the DataSet and use the getRow() method to get the current row.
 */
public interface DataSet extends Closeable, Iterable<Row> {
   ...
   *
   * @param item
   * @return the index (0-based) of the SelectItem or -1 if the SelectItem doesn't exist in this
   *         DataSet.
   */
  ...
  /**
   * Converts the DataSet into an Iterator. Note that unlike many {@link Iterable} objects,
   * {@link DataSet}s are unlikely to allow creation of multiple iterators without risking loss of
   * data in each individual iteration loop.
   */
  @Override
