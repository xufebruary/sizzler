 ...
 * Subclasses should implement the {@link #decorateIdentity(List)} method to have
 * {@link #equals(Object)} and {@link #hashCode()} automatically implemented.
 */
public abstract class BaseObject {
  ...
  /**
   * Override this method if the equals method should support different subtypes. For example, if
   * different subtypes of Number should be supported, implement this method with:
   * <p/>
   * <code>
   * obj instanceof Number
   * </code>
   * <p/>
   * and make sure that the decorateIdentity(...) method will always return a comparable list of
   * identity-objects.
   *
   * @param obj
    ...
    if (obj instanceof BaseObject) {
      ...
      if (classEquals(that)) {
        ...
        if (list1.size() != list2.size()) {
          throw new IllegalStateException("Two instances of the same class ("
              + getClass().getName() + ") returned different size decorated identity lists");
        }
  ...
  /**
   * Subclasses should implement this method and add all fields to the list that are to be included
   * in equals(...) and hashCode() evaluation
   *
   * @param identifiers
   */
