...
public class QueryParser {
  ...
  public Query parse() throws QueryParserException {
    ...
    // parse FROM
    {
      final String fromClause =
          getSubstring(
              ...
              getNextStartIndex(whereIndices, groupByIndices, havingIndices, orderByIndices,
                  limitIndices, offsetIndices));
    ...
    if (whereIndices != null) {
      final String whereClause =
          getSubstring(
              getLastEndIndex(whereIndices),
              getNextStartIndex(groupByIndices, havingIndices, orderByIndices, limitIndices,
                  offsetIndices));
      if (whereClause != null) {
    ...
    if (groupByIndices != null) {
      final String groupByClause =
          getSubstring(getLastEndIndex(groupByIndices, whereIndices),
    ...
    if (havingIndices != null) {
      final String havingClause =
          getSubstring(
              getLastEndIndex(havingIndices, groupByIndices, whereIndices, fromIndices,
                  selectIndices), getNextStartIndex(orderByIndices, limitIndices, offsetIndices));
    ...
    if (orderByIndices != null) {
      final String orderByClause =
          getSubstring(
              getLastEndIndex(orderByIndices, havingIndices, groupByIndices, whereIndices,
                  fromIndices, selectIndices), getNextStartIndex(limitIndices, offsetIndices));
    ...
    if (limitIndices != null) {
      final String limitClause =
          getSubstring(
              getLastEndIndex(limitIndices, orderByIndices, havingIndices, groupByIndices,
                  whereIndices, fromIndices, selectIndices), getNextStartIndex(offsetIndices));
      if (limitClause != null) {
    ...
    if (offsetIndices != null) {
      final String offsetClause =
          getSubstring(
              getLastEndIndex(offsetIndices, limitIndices, orderByIndices, havingIndices,
                  groupByIndices, whereIndices, fromIndices, selectIndices), getNextStartIndex());
  ...
  private void parseFromClause(Query query, String fromClause) {
    QueryPartParser clauseParser =
        new QueryPartParser(new FromItemParser(_dataContext, query), fromClause, ",");
    clauseParser.parse();
  }

  private void parseSelectClause(Query query, String selectClause) {
    QueryPartParser clauseParser =
        new QueryPartParser(new SelectItemParser(query, false), selectClause, ",");
    clauseParser.parse();
  }
  ...
  private void parseWhereClause(Query query, String whereClause) {
    ...
    QueryPartParser clauseParser =
        new QueryPartParser(new WhereItemParser(query), whereClause, " AND ");
    clauseParser.parse();
  }

  private void parseGroupByClause(Query query, String groupByClause) {
    QueryPartParser clauseParser =
        new QueryPartParser(new GroupByItemParser(query), groupByClause, ",");
    clauseParser.parse();
  }
  ...
  private void parseHavingClause(Query query, String havingClause) {
    ...
    // compound filter items at 2nd level parsing
    QueryPartParser clauseParser =
        new QueryPartParser(new HavingItemParser(query), havingClause, " AND ");
    clauseParser.parse();
  }

  private void parseOrderByClause(Query query, String orderByClause) {
    QueryPartParser clauseParser =
        new QueryPartParser(new OrderByItemParser(query), orderByClause, ",");
    clauseParser.parse();
  }
  ...
  /**
   * Finds the start and end indexes of a string in the query. The string parameter of this method
   * is expected to be in upper case, while the query itself is tolerant of case differences.
   *
   * @param string
