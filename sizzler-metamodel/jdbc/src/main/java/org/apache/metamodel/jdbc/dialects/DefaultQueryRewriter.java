...
/**
 * Generic query rewriter that adds syntax enhancements that are only possible to resolve just
...
public class DefaultQueryRewriter extends AbstractQueryRewriter {
    ...
    if (operand != null) {
      if (operand instanceof String) {
        ...
        if (str.indexOf('\'') != -1) {
          ...
          FilterItem replacementFilterItem =
              new FilterItem(item.getSelectItem(), item.getOperator(), str);
        ...
        FilterItem replacementFilterItem =
            new FilterItem(item.getSelectItem(), item.getOperator(), elements);
        return super.rewriteFilterItem(replacementFilterItem);
      }
    }
   ...
   * 
   * This default implementation returns the JDBC spec's escape syntax for a timestamp: {ts
