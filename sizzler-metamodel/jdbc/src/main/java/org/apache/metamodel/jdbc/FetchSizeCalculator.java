 ...
 * 
 * The approach used in this class is largely based on the documentation of Oracle's caching size,
 * see <a href= "http://www.oracle.com/technetwork/database/enterprise-edition/memory.pdf" >JDBC
 * Memory Management</a>, section "Where does it all go?".
 */
final class FetchSizeCalculator {

  /**
   * 22 bytes is a reasonable approximation for remaining row types, we add a few bytes to be on the
   * safe side.
   */
   ...
   * @return an integer representing how many rows to eagerly fetch for the query
   */
  public int getFetchSize(Query query) {
    ...
    if (maxRows != null && result > maxRows) {
      logger.debug("Result ({}) was below max rows ({}), adjusting.", result, maxRows);
      result = maxRows;
    }
    return result;
  }
  ...
  /**
   * Gets whether a query is guaranteed to only yield a single row. Such queries are queries that
   * only consist of aggregation functions and no group by clause.
   * 
   * @param query
   * @return
   */
   ...
   * 
   * @param query the query that will yield the rows
   * @return an integer representing the size of a row from the given query (in bytes).
   */
   ...
   * @return an integer representing the size of a row with the given columns (in bytes).
   */
   ...
   * 
   * @param bytesPerRow the size (in bytes) of a single row in the result set.
  ...
  protected int getFetchSize(int bytesPerRow) {
    ...
    if (result < MIN_FETCH_SIZE) {
      logger.debug("Result ({}) was below minimum fetch size ({}), adjusting.", result,
          MIN_FETCH_SIZE);
      result = MIN_FETCH_SIZE;
    } else if (result > MAX_FETCH_SIZE) {
      logger.debug("Result ({}) was above maximum fetch size ({}), adjusting.", result,
          MAX_FETCH_SIZE);
  ...
  /**
   * Gets the size (in bytes) of a column with a specific {@link ColumnType} and size
   */
  ...
  /**
   * Gets the (approximate) size (in bytes) of a column with a specific {@link ColumnType}.
   */
