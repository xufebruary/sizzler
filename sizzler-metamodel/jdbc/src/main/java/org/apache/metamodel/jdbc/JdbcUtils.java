...
public final class JdbcUtils {
  ...
  public static MetaModelException wrapException(SQLException e, String actionDescription)
  ...
  /**
   * Method which handles the action of setting a parameterized value on a statement. Traditionally
   * this is done using the {@link PreparedStatement#setObject(int, Object)} method but for some
   ...
   * @throws SQLException
   */
  public static void setStatementValue(final PreparedStatement st, final int valueIndex,
      final Column column, Object value) throws SQLException {
    ...
    for (FilterItem whereItem : whereItems) {
      ...
      if (!inlineValues) {
        if (isPreparedParameterCandidate(whereItem)) {
          ...
          whereItem =
              new FilterItem(whereItem.getSelectItem(), whereItem.getOperator(),
                  new QueryParameter());
        }
      }
  ...
  /**
   * Determines if a particular {@link FilterItem} will have it's parameter (operand) replaced
   * during SQL generation. Such filter items should succesively have their parameters set at
   * execution time.
   * 
   * @param whereItem
   * @return
