...
/**
 * Abstract implementation of query rewriter. This implementation delegates the rewriting of the
 * Query into several subtasks according to the query items to be rendered. This makes it easy to
 * overload single methods in order to correct syntax quirks.
 */
public abstract class AbstractQueryRewriter implements IQueryRewriter {
  ...
  /**
   * Method to modify query before rewriting begins. Overwrite this method if you want to change
   * parts of the query that are not just rendering related. Cloning the query before modifying is
   * recommended in order to not violate referential integrity of clients (the query is mutable).
   * 
   * @param strategy
   * @param query
