...
/**
 * Implementation of {@link CompiledQuery} for JDBC which uses a {@link PreparedStatement} behind
 * the scenes.
 */
final class JdbcCompiledQuery extends DefaultCompiledQuery implements CompiledQuery {
  ...
  public JdbcCompiledQuery(JdbcDataContext dc, Query query) {
    ...
    config.maxActive =
        getSystemPropertyValue(JdbcDataContext.SYSTEM_PROPERTY_COMPILED_QUERY_POOL_MAX_SIZE, -1);
    ...
    config.timeBetweenEvictionRunsMillis =
        getSystemPropertyValue(
            JdbcDataContext.SYSTEM_PROPERTY_COMPILED_QUERY_POOL_TIME_BETWEEN_EVICTION_RUNS_MILLIS,
            1000);

    _pool =
        new GenericObjectPool<JdbcCompiledQueryLease>(new JdbcCompiledQueryLeaseFactory(dc, _sql),
            config);
    _closed = false;
  ...
  public JdbcCompiledQueryLease borrowLease() {
    if (logger.isDebugEnabled()) {
      logger.debug("Borrowing lease. Leases (before): Active={}, Idle={}", getActiveLeases(),
          getIdleLeases());
    }
    try {
  ...
  public void returnLease(JdbcCompiledQueryLease lease) {
    if (logger.isDebugEnabled()) {
      logger.debug("Returning lease. Leases (before): Active={}, Idle={}", getActiveLeases(),
          getIdleLeases());
    }
    try {
