...
/**
 * The QuerySplitter class makes it possible to split up queries that are expected to yield a huge
 * result set which may cause performance problems like OutOfMemoryError's or very long processing
 * periods. The resulting queries will in union produce the same result, but in smaller bits
 * (resultsets with less rows).
 * 
 * Note that there is an initial performance-penalty associated with splitting the query since some
 * queries will be executed in order to determine reasonable intervals to use for the resulting
...
public final class QuerySplitter {
   ...
   * 
   * @return a list of queries that can be executed to yield the same collective result as this
   *         QuerySplitter's query
   */
  ...
  private List<Query> splitQueryBasedOnColumn(Column column) {
    ...
    if (wholeRange <= 1) {
      ...
      for (int i = 0; i < numSplits; i++) {
        ...
        FilterItem lowerThanFilter =
            new FilterItem(new SelectItem(column), OperatorType.LESS_THAN, highLimit);
        FilterItem higherThanFilter =
            new FilterItem(new SelectItem(column), OperatorType.GREATER_THAN, lowLimit);
        FilterItem equalsFilter =
            new FilterItem(new SelectItem(column), OperatorType.EQUALS_TO, lowLimit);

        if (i == 0) {
          // This is the first split query: no higherThan filter and
          // include
          // IS NULL
          FilterItem nullFilter =
  ...
  /**
   * Sets the desired maximum result set row count. Note that this size cannot be guaranteed, but
   * will serve as an indicator for determining the split-size
   * 
   * @param maxRows
   */
