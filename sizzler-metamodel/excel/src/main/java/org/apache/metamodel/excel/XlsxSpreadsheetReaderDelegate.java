...
/**
 * {@link SpreadsheetReaderDelegate} implementation for the "new" XLSX format. This implementation
 * is very efficient as it uses SAX XML parsing which does not bloat memory usage in the same way
 * that POI's user model does.
 */
final class XlsxSpreadsheetReaderDelegate implements SpreadsheetReaderDelegate {
  ...
  public Schema createSchema(String schemaName) throws Exception {
    ...
    try {
      ...
      final XlsxWorkbookToTablesHandler workbookToTables =
          new XlsxWorkbookToTablesHandler(schema, _tableNamesToInternalIds);
  ...
  public void notifyTablesModified() {
    final XlsxWorkbookToTablesHandler workbookToTables =
        new XlsxWorkbookToTablesHandler(null, _tableNamesToInternalIds);
    try {
      final OPCPackage pkg = openOPCPackage();
      try {
        final XSSFReader xssfReader = new XSSFReader(pkg);
  ...
  private DataSet buildDataSet(final Column[] columns, int maxRows, final String relationshipId,
    ...
    final XlsxRowPublisherAction publishAction =
        new XlsxRowPublisherAction(_configuration, columns, relationshipId, xssfReader);

    return new RowPublisherDataSet(selectItems.toArray(new SelectItem[selectItems.size()]),
        maxRows, publishAction, new Closeable() {
          @Override
  ...
  private void buildColumns(final MutableTable table, final String relationshipId,
    ...
    final XlsxSheetToRowsHandler handler =
        new XlsxSheetToRowsHandler(rowCallback, xssfReader, _configuration);
  ...
  protected void buildColumns(final MutableTable table, final List<String> columnNames) {
    ...
    for (String columnName : columnNames) {
      if (columnName != null || !_configuration.isSkipEmptyColumns()) {
        ...
        table
            .addColumn(new MutableColumn(columnName, ColumnType.STRING, table, columnNumber, true));
      }
      columnNumber++;
    }
  }

  private void buildTables(final XSSFReader xssfReader,
      final XlsxWorkbookToTablesHandler workbookToTables) throws Exception {
