...
/**
 * The default {@link SpreadsheetReaderDelegate}, which uses POI's main user model to read
 * spreadsheets: the Workbook class.
 */
final class DefaultSpreadsheetReaderDelegate implements SpreadsheetReaderDelegate {

  private static final Logger logger = LoggerFactory
      .getLogger(DefaultSpreadsheetReaderDelegate.class);
  ...
  private MutableTable createTable(final Workbook wb, final Sheet sheet) {
    ...
    // 如果没有指定 列名，则直接使用 A,B...Z,AA....ZZ做为列名
    if (columnNameLineNumber == ExcelConfiguration.NO_COLUMN_NAME_LINE) {
      ...
      // are!)
      // 定位到第一个有数据的row，然后根据该row所包含的column数
      while (row == null && rowIterator.hasNext()) {
        row = rowIterator.next();
      }
  ...
  /**
   * Gets the column offset (first column to include). This is dependent on the row used for column
   * processing and whether the skip empty columns property is set.
   * 
   * @param row
   * @return
   */
