...
class XlsxRowPublisherAction implements Action<RowPublisher> {
  ...
  public XlsxRowPublisherAction(ExcelConfiguration configuration, Column[] columns,
      String relationshipId, XSSFReader xssfReader) {
  ...
  public void run(final RowPublisher publisher) throws Exception {
    ...
    final XlsxSheetToRowsHandler handler =
        new XlsxSheetToRowsHandler(rowCallback, _xssfReader, _configuration);
