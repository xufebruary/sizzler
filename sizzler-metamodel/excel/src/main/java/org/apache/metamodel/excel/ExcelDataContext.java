 ...
 * 
 * This DataContext supports both the "old" .xls format and the "new" .xlsx format, and saves the
 * user the trouble of figuring out which one to use, simply by detecting it at runtime and
 * delegating to the appropriate implementation.
 */
public final class ExcelDataContext extends QueryPostprocessDataContext implements
  ...
  private SpreadsheetReaderDelegate getSpreadsheetReaderDelegate() throws MetaModelException {
    if (_spreadsheetReaderDelegate == null) {
      synchronized (this) {
        if (_spreadsheetReaderDelegate == null) {
          _spreadsheetReaderDelegate =
              _resource.read(new Func<InputStream, SpreadsheetReaderDelegate>() {
                @Override
