...
/**
 * Exception thrown when a line in a CSV file has an inconsistent amount of columns compared to the
 * previous lines (and headers). The exception will be thrown when {@link DataSet#next()} is called.
 * 
 * Note that this exception is only thrown if the
 * {@link CsvConfiguration#isFailOnInconsistentRowLength()} property is true. Enabling it allows a
 * somewhat different approach to iterating through a resulting DataSet. For example something like:
 * 
 * <pre>
 * while (true) {
 *   try {
...
public final class InconsistentRowLengthException extends InconsistentRowFormatException {
  ...
  public InconsistentRowLengthException(int columnsInTable, Row proposedRow, String[] line,
      int rowNumber) {
  ...
  public String getMessage() {
    return "Inconsistent length of row no. " + getRowNumber() + ". Expected " + getColumnsInTable()
        + " columns but found " + getColumnsInLine() + ".";
  }
   ...
   * 
   * @return an int representing the amount of values in the inconsistent line.
   */
