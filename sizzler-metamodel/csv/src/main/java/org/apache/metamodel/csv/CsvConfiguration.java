...
public final class CsvConfiguration extends BaseObject implements Serializable {
  ...
  /**
   * The value is '\\uFFFF', the "not a character" value which should not occur in any valid Unicode
   * string. This special char can be used to disable either quote chars or escape chars.
   */
  ...
  public CsvConfiguration(char separatorChar) {
    this(DEFAULT_COLUMN_NAME_LINE, FileHelper.DEFAULT_ENCODING, separatorChar, DEFAULT_QUOTE_CHAR,
        DEFAULT_ESCAPE_CHAR);
  }

  public CsvConfiguration(int columnNameLineNumber) {
    this(columnNameLineNumber, FileHelper.DEFAULT_ENCODING, DEFAULT_SEPARATOR_CHAR,
        DEFAULT_QUOTE_CHAR, DEFAULT_ESCAPE_CHAR);
  }

  public CsvConfiguration(int columnNameLineNumber, boolean failOnInconsistentRowLength,
      boolean multilineValues) {
    this(columnNameLineNumber, FileHelper.DEFAULT_ENCODING, DEFAULT_SEPARATOR_CHAR,
        DEFAULT_QUOTE_CHAR, DEFAULT_ESCAPE_CHAR, failOnInconsistentRowLength, multilineValues);
  }

  public CsvConfiguration(int columnNameLineNumber, String encoding, char separatorChar,
  ...
  public CsvConfiguration(int columnNameLineNumber, String encoding, char separatorChar,
    ...
    this(columnNameLineNumber, encoding, separatorChar, quoteChar, escapeChar,
  ...
  /**
   * Determines whether to fail (by throwing an {@link InconsistentRowLengthException}) if a line in
   * the CSV file has inconsistent amounts of columns.
   * 
   * If set to false (default) MetaModel will gracefully fill in missing null values in or ignore
   * additional values in a line.
   * 
   * @return a boolean indicating whether to fail or gracefully compensate for inconsistent lines in
   *         the CSV files.
   */
  ...
  /**
   * Determines whether the CSV files read using this configuration should be allowed to have
   * multiline values in them.
   * 
   * @return
   */
  ...
  public String toString() {
    return "CsvConfiguration[columnNameLineNumber=" + columnNameLineNumber + ", encoding="
        + encoding + ", separatorChar=" + separatorChar + ", quoteChar=" + quoteChar
        + ", escapeChar=" + escapeChar + ", failOnInconsistentRowLength="
