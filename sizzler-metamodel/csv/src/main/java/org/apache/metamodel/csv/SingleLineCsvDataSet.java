...
/**
 * A specialized DataSet implementation for the CSV module under circumstances where multiline
 * values are disabled. In this case we can use a optimized CSVParser and also lazy evaluate lines
 * read from the file.
 */
final class SingleLineCsvDataSet extends AbstractDataSet {
  ...
  public SingleLineCsvDataSet(BufferedReader reader, CSVParser csvParser, Column[] columns,
      Integer maxRows, int columnsInTable, boolean failOnInconsistentRowLength) {
    super(columns);
    _reader = reader;
  ...
  public boolean nextInternal() {
    ...
    try {
      ...
      _rowNumber++;
      _row =
          new SingleLineCsvRow(this, line, _columnsInTable, _failOnInconsistentRowLength,
              _rowNumber);
      return true;
    } catch (IOException e) {
      close();
      throw new MetaModelException("IOException occurred while reading next line of CSV resource",
          e);
    }
  }
