 ...
 * @see
 */
@Service("standardModelDataService")
public class StandardModelDataServiceImpl implements StandardModelDataService {
  ...
  @Autowired
  private StandardModelDataUtil standardModelDataUtil;
  ...
  public List<PtoneVariableData> getData(PtoneWidgetInfo ptoneWidgetInfo,
    ...
    Map<String, PtoneMetricsDimension> metricsDimensionMap = new LinkedHashMap<String, PtoneMetricsDimension>();

    UserConnection userConnection = ptoneWidgetParam.getUserConnection();
    String tableId = gaWidgetInfo.getProfileId();
    UserConnectionSourceDto sourceDto = dataSourceManagerService
        .getSourceDtoByTableIdIncludeTable(tableId);
    ...
    List<PtoneMetricsDimension> dimensionsList = commonDataUtil.fixModelDimensionList(
        dimensionData, ptoneWidgetParam, xAxisDateDimension, metricsDimensionMap);
    List<String> dimensionsKeyList = ptoneWidgetParam.getDimensionsKeyList();

    // 修正指标列表
    List<PtoneMetricsDimension> metricsList = commonDataUtil.fixModelMetricsList(metricsData,
        ptoneWidgetParam, metricsDimensionMap);
    ...
    Map<String, String> dateMap = PtoneDateUtil.getInstance(
        currentUserCache.getCurrentUserWeekStartSetting()).getStartEndDate(dateKey, "yyyy-MM-dd");
    ...
    // 目标值
    Number goals = (ptoneWidgetInfo.getTargetValue() == null || ptoneWidgetInfo.getTargetValue()
        .equalsIgnoreCase("")) ? null : Double.parseDouble(ptoneWidgetInfo.getTargetValue());

    // 为line时使用datetime类型x轴
    boolean useDatetimeAxis = (GraphType.LINE.equals(GraphType.valueOf(graphType.toUpperCase()))
    ...
    boolean judgeMulitY = String.valueOf(true).equals(
        webParamMap.get(Constants.PARAM_JUDGE_MULTI_Y));
    ...
    if (judgeMulitY
      ...
      // mulitYUseDatePeriod = (dimensionsKeyList != null &&
      // dimensionsKeyList.size() > 0);
    ...
    if (GraphType.TABLE.equals(GraphType.valueOf(graphType.toUpperCase()))) {
      ...
      if (StringUtil.isNotBlank(sortParam)) {
        sort = standardModelDataUtil.parseSort(sortParam, dsCode);
      } else if (StringUtil.isNotBlank(sort)) {
        sort = standardModelDataUtil.parseSort(sort, dsCode);
      }
    ...
    // 创建查询参数对象
    StandardModelQueryParam standardModelQueryParam = new StandardModelQueryParam();
    standardModelQueryParam.setFileId(fileId);
    standardModelQueryParam.setTableId(tableId);
    standardModelQueryParam.setTableName(tableName);
    standardModelQueryParam.setTableCode(tableCode);
    standardModelQueryParam.setMetricsList(metricsList);
    standardModelQueryParam.setDimensionsList(dimensionsList);
    standardModelQueryParam.setMetricsDimensionMap(metricsDimensionMap);
    standardModelQueryParam.setDateDimension(dateDimension);
    standardModelQueryParam.setxAxisDateDimension(xAxisDateDimension);
    standardModelQueryParam.setFilters(filters);
    standardModelQueryParam.setSort(sort);
    standardModelQueryParam.setStartDate(startDate);
    standardModelQueryParam.setEndDate(endDate);
    standardModelQueryParam.setUseDateDimensionInSelect(useDatetimeAxis); // 判断时间维度是否作为查询维度（在使用时间轴时，用户没有时间维度将时间维度增加到select中）
    standardModelQueryParam.setDateKey(dateKey);
    standardModelQueryParam.setDatePeriod(datePeriod);
    standardModelQueryParam.setCurrentUserCache(currentUserCache);
    standardModelQueryParam.setSource(sourceDto.parseToSource());
    standardModelQueryParam.setHdfsPath(hdfsPath);
    standardModelQueryParam.setSchema(schema);

    // 查询数据，并对查询到的结果数据进行处理
    ModelData modelData = null;
    ModelData lastModelData = null;
    try {
      modelData = standardModelDataUtil.getModelData(standardModelQueryParam, ptoneWidgetParam);
      if (chartDataType.equalsIgnoreCase(ChartDataType.QOQNUMBER.toString())) {
        // 如果是number环比类型图表，需要获取上一期数值
        lastModelData = standardModelDataUtil.getLastData(dateKey, currentUserCache,
            ptoneWidgetParam, standardModelQueryParam);
      }
    ...
    Map<String, Map<String, Object>> metricsTotalsMap = commonDataUtil.getModelMetricsTotalsMap(
        modelData, ptoneWidgetParam);
    ...
    // 将返回的数据 转换为对应的图表的格式
    GraphVariableDataDesc graphDesc = new GraphVariableDataDesc(ChartDataType.valueOf(chartDataType
        .toUpperCase()), modelData);
    ...
    graphDesc.setQueryParam(standardModelQueryParam);
    graphDesc.setLastModelData(lastModelData);
    List<PtoneVariableData> ptoneVariableDataList = PtoneDispatcher.getInstance().dispatch(
        graphDesc);
    ...
    for (PtoneVariableData ptoneVariableData : ptoneVariableDataList) {
      ...
      boolean isWeekOrQuarter = xAxisDateDimension != null ? StandardModelDataUtil
          .notUsexAxisDate(xAxisDateDimension.getDatePeriod()) : false;
