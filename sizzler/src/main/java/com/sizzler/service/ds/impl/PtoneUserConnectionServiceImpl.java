...
public class PtoneUserConnectionServiceImpl extends ServiceBaseInterfaceImpl<UserConnection, Long>
  ...
  public void saveAndUpdateOthersConfig(UserConnection userConnection) {
    ...
    UserConnection dbConnection =
        ptoneUserConnectionDao.get(userConnection.getName(), userConnection.getUid(),
            userConnection.getDsId(), userConnection.getSpaceId());
  ...
  public List<DsContentView> getSpaceDsContentView(String spaceId,String userSource) {
    List<DsContentView> dsContentViewList = ptoneUserConnectionDao.getSpaceDsContentView(spaceId,userSource);
    // 修正upload数据源的文件统计数
  ...
  public ConnectionTimezoneVo getConnectionTimezone(long dsId, String connectionId, String sourceId) {
    ...
    if(DsConstants.isApiDs(dsInfo.getCode())){
      ...
      UserConnectionSource source = serviceFactory.getUserConnectionSourceService().getSource(sourceId);
      String dataTimezone = source.getDataTimezone();
      if(StringUtil.isNotBlank(dataTimezone)){
        vo.setDataTimezone(JSON.parseObject(dataTimezone,TimezoneVo.class));
      }
      //对于非api类型的数据源需要查询每个字段是否存在timestamp类型
      List<UserConnectionSourceTableColumn> columns = serviceFactory.getUserConnectionSourceTableColumnService().findBySourceId(sourceId);
      String hasTimezoneFiled = Constants.inValidate;
      if(dsInfo.getSupportTimezone().equals(Constants.validate) && CollectionUtil.isNotEmpty(columns)){
        for(UserConnectionSourceTableColumn column:columns){
          if(StringUtil.isNotBlank(column.getDataType()) && column.getDataType().equals("TIMESTAMP")){
  ...
  @Override
  public void updateConnectionTimezone(long dsId, String connectionId, String sourceId,ConnectionTimezoneVo vo) {
    PtoneDsInfo dsInfo = serviceFactory.getPtoneDsInfoCache().getPtoneDsInfoById(dsId);
    if(DsConstants.isApiDs(dsInfo.getCode())){
  ...
  public String getSourceIdByTableId(String tableId) {
    UserConnectionSourceTableDto tableDto =
        serviceFactory.getUserConnectionSourceTableService().getByTableIdOfDto(tableId);
    if (tableDto == null) {
      return null;
    }
