...
public class BasicSqlFormatter {
  ...
  private static Boolean upperCase(String token) {
    String lToken = token.toLowerCase();
    return (BEGIN_CLAUSES.contains(lToken) || END_CLAUSES.contains(lToken)
        || LOGICAL.contains(lToken) || QUANTIFIERS.contains(lToken) || DML.contains(lToken) || MISC
          .contains(lToken));
  }
  ...
  private static class FormatProcess {
      ...
      while (tokens.hasMoreTokens()) {
        ...
        if ("'".equals(token)) {
          ...
          do {
            t = tokens.nextToken();
            token += t;
          } while (!"'".equals(t) && tokens.hasMoreTokens()); // cannot handle
                                                              // single quotes
    ...
    private void out() {
      result.append(upperCase(token) ? token.toUpperCase() : token);
    }
