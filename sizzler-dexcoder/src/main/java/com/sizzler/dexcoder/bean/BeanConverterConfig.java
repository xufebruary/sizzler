...
public class BeanConverterConfig {
  ...
  /**
   * 获取实例，提供的功能应用于 {@link BeanConverter}. 这是一个伪单例 -
   * 每一个线程的ContextClassLoader提供一个单例的实例 这种机制提供了在同一个web容器中部署的应用程序之间的隔离
   * 
  ...
  /**
   * 设置实例，提供的功能应用于 {@link BeanConverter}. 这是一个伪单例 -
   * 每一个线程的ContextClassLoader提供一个单例的实例 这种机制提供了在同一个web容器中部署的应用程序之间的隔离
   * 
   * @param newInstance
   *          该伪单例的实例 BeanConverterConfig
   */
