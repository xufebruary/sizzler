...
/**
 * 该类的一个实例表示为线程的ContextClassLoader提供一个值。 <br/>
 * 有时需要将值存储在全局变量“global”，（包括使用的单例模式） <br/>
 * 在只有一个classloader的应用程序，数据可以简单的存储在一些类的“static”成员。 当涉及多个classloader，该方法可能会失败。
 ...
 * 
 * <p>
 * <strong>注意:</strong> 这个类需要小心一些，确保使用这个类的组件由容器来“取消部署”，
 * 特定的组件classloader和所有相关联的类（和他们的表态变量）进行垃圾回收。
 * 
 * 不幸的是有一个场景这个类不会正常工作，更不幸的是没有已知的解决方法： 当组件被取消部署，组件（或容器）调用该类实例的“unset”方法
 * 
 * 该问题发生于：
 * <ul>
...
public class ContextClassLoaderLocal {
  ...
  /**
   * 返回这个 ContextClassLoaderLocal 的初始值变量 每一次调用该方法的为每一个 ContextClassLoaderLocal 的
   * ContextClassLoader 首次访问为get或set。 如果程序员希望 ContextClassLoaderLocal
   * 变量被初始化为null以外的一些值， ContextClassLoaderLocal 必须被继承并且该方法被重写。 通常情况下，使用一个匿名的内部类。
