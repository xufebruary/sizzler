...
import com.sizzler.dexcoder.exceptions.AssistantException;
import com.sizzler.dexcoder.utils.ClassUtils;
...
public class SqlAssembleUtils {
  ...
  public static BoundSql buildInsertSql(Object entity, Criteria criteria, NameHandler nameHandler) {
    ...
    String tableName = nameHandler.getTableName(entityClass).toLowerCase();
  ...
  private static List<AutoField> getEntityAutoField(Object entity, int operateType) {
    ...
    try {
      for (PropertyDescriptor pd : pds) {
        ...
        Annotation annotation = entity.getClass().getDeclaredField(fieldName)
      ...
      e.printStackTrace();
      LOG.error("getEntityAutoField error.|" + e);
    }
    return autoFieldList;
  }
  ...
  public static BoundSql buildDeleteSql(Class<?> clazz, Long id, NameHandler nameHandler) {
    ...
    List<Object> params = new ArrayList<Object>();
    params.add(id);
    String tableName = nameHandler.getTableName(clazz).toLowerCase();
  ...
  public static String buildColumnSql(Class<?> clazz, NameHandler nameHandler,
    ...
    try {
      for (PropertyDescriptor pd : pds) {
        ...
        Annotation annotation = clazz.getDeclaredField(fieldName).getAnnotation(Ignore.class);
        if (annotation != null) {
          continue;
        }
