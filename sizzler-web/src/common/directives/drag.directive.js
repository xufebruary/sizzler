/**
 * Created by jianqing on 16/5/18.
 */
...
var isIE6 = !('minWidth' in dragVars.html.style),
    isLosecapture = !isIE6 && 'onlosecapture' in dragVars.html,
    isSetCapture = 'setCapture' in dragVars.html;

function noop() {}
function preventDefault() {return false;}
var eventTypes = {
    start: dragVars.isTouch ? 'touchstart' : 'mousedown',
    move: dragVars.isTouch ? 'touchmove' : 'mousemove',
    end: dragVars.isTouch ? 'touchend' : 'mouseup'
};

var touchId = 0;
var getEvent = dragVars.isTouch ? function(event, touchId) {
...
Drag.prototype = {
    ...
    start: function(event) {
        ...
        if (isLosecapture) {
            ...
            dragVars.$window.on('blur', this.end);
        }
    ...
    move: function(event) {
        ...
        if (isLosecapture) {
            ...
            dragVars.$window.off('blur', this.end);
        }
...
Drag.create = function(elem, event, GPU) {
    ...
    dragEvent.onstart = function(event) {
        var $wrap = elem.parentNode.nodeName === 'BODY' ?
            dragVars.$document : $elem.offsetParent();

        var isFixed = $elem.css('position') === 'fixed';
        var position = $elem.position();
        var ww = dragVars.$window.width();
        var wh = dragVars.$window.height();
...
export function dragHandleDirective(){
	return {
		restrict: 'A',
		link:link
	};
	function link(scope, elem){
		...
		if($(dragNode).hasClass('custom-widget')){
			var widget = $(dragNode).parents('.widget');
			//拖拽自定义widget的前提条件是，有创建自定义widget的权限，在编辑模式下，打开了属于自己的widget编辑器
			elem.on(Drag.START, function(event) {
				var canDrag = widget.hasClass('can-edit-custom') && widget.hasClass('is-self-editor-open');
				if(canDrag){
