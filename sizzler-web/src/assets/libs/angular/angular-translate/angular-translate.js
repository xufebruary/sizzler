...
angular.module('pascalprecht.translate').provider('$translate', ['$STORAGE_KEY', function ($STORAGE_KEY) {
   ...
   *  });
    ...
    var flatResult = function(result){
        var val,key,replaceStr;
        if(isDomain('ptone.com.cn')){
            replaceStr = "PTONE_COMCN_DOMAIN"
        }else if(isDomain('ptone.cn')){
            replaceStr = "PTONE_CN_DOMAIN"
        }else if(isDomain('ptone.jp')){
            replaceStr = "PTONE_JP_DOMAIN"
        }else if(isDomain('datadeck.jp')){
            replaceStr = "DATADECK_JP_DOMAIN"
        }else if(isDomain('datadeck.com')){
            replaceStr = "DATADECK_COM_DOMAIN"
        }else if(isDomain('datadeck.cn')){
            replaceStr = "DATADECK_CN_DOMAIN"
        }
        for( key in result){
            if(key.indexOf(replaceStr) > -1){
                val = result[key];
                var newKey = key.replace("." + replaceStr,"");
                result[newKey] = val;
            }
        }
        return result;
   ...
   *  });
    ...
    this.$get = [
        ...
        function ($log, $injector, $rootScope, $q) {
            ...
            $translate.getI18nMap = function () {
                return i18nKeyMap;
            };

            $translate.initI18nMap = function(){
                for(var o in i18nKeyMap){
                    initI18nKey(o);
                }
            }

            //得到所有国际化key
            var initI18nKey = function (key) {
                ...
                $injector.get($loaderFactory)(loaderOptions).then(function (data) {
                    ...
                    i18nKeyMap[key] = translationTable;
                });
