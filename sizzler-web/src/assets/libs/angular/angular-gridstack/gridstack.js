...
(function(factory) {
    if (typeof define === 'function' && define.amd) {
        define(['jquery', 'lodash', 'jquery-ui/ui/data', 'jquery-ui/ui/disable-selection', 'jquery-ui/ui/focusable',
    ...
    GridStackEngine.prototype.addNode = function(node, triggerAddEvent) {
        ...
        //只有当真正新增及widget拖拽出边缘放手后回显时,需要做碰撞检测
		if(node.autoPosition || jQuery(node.el[0]).hasClass('ui-draggable-dragging')) {
    ...
    GridStack.prototype._updateStyles = function(maxHeight) {
        ...
        if (!this.opts.verticalMargin || this.opts.cellHeightUnit === this.opts.verticalMarginUnit) {
            getHeight = function(nbRows, nbMargins, type) {
                var value = self.opts.cellHeight * nbRows;
                if(type == 'top' && nbMargins != 0){
                    value += self.opts.verticalMargin;
                }
                return value + self.opts.cellHeightUnit;
        ...
        if (maxHeight > this._styles._max) {
            for (var i = this._styles._max; i < maxHeight; ++i) {
                Utils.insertCSSRule(this._styles,
                    ...
                    'height: ' + getHeight(i + 1, i, 'height') + ';',
                    i
                );
                Utils.insertCSSRule(this._styles,
                    prefix + '[data-gs-min-height="' + (i + 1) + '"]',
                    'min-height: ' + getHeight(i + 1, i, 'minHeight') + ';',
                    i
                );
                Utils.insertCSSRule(this._styles,
                    prefix + '[data-gs-max-height="' + (i + 1) + '"]',
                    'max-height: ' + getHeight(i + 1, i, 'minHeight') + ';',
                    i
                );
                Utils.insertCSSRule(this._styles,
                    prefix + '[data-gs-y="' + i + '"]',
                    'top: ' + getHeight(i, i, 'top') + ';',
                    i
                );
            }
    ...
    GridStack.prototype._updateContainerHeight = function() {
        ...
        if (!this.opts.verticalMargin) {
            ...
            this.container.css('height', (height * this.opts.cellHeight -
                this.opts.verticalMargin) + this.opts.cellHeightUnit);
        } else {
