    ...
    app.directive('gridstackItem', ['$timeout', function ($timeout) {
        ...
        return {
            ...
            link: function (scope, element, attrs, controller) {
                ...
                $(element).attr('data-gs-id', scope.gsItemWidgetId);
                ...
                if(scope.gsItemAutopos){
                    $(element).attr('data-gs-auto-position', scope.gsItemAutopos);
                }

                scope.$watch('gsItemNoResize', function (val) {
                    if(val){
                        $(element).attr('data-gs-no-resize', 'true');
                    } else {
                        $(element).removeAttr('data-gs-no-resize')
                    }
                });

                if(scope.gsItemNoResize){
                    $(element).attr('data-gs-no-resize', 'true');
                } else {
                    $(element).removeAttr('data-gs-no-resize')
                }

                if(scope.gsItemNoMove){
                    $(element).attr('data-gs-no-move', 'true');
                } else {
                    $(element).removeAttr('data-gs-no-move')
                }
